...
class _PharmacyMainScreenState extends ConsumerState<PharmacyMainScreen> {
  ...
  void _syncControllers() {
    ...
    // Ensure controllers list matches medications length
    ...
    for (int i = 0; i < meds.length; i++) {
      ...
      if (times[i] != TimeOfDay.fromDateTime(meds[i].time)) {
        times[i] = TimeOfDay.fromDateTime(meds[i].time);
      }
  ...
  void _updateMedicationFromForm(int index) {
    final newMed = Medication(
      id: ref.read(pharmacyProvider).medications[index].id,
      ...
      time: times[index] != null
          ? DateTime(0, 0, 0, times[index]!.hour, times[index]!.minute)
          : DateTime.now(),
      frequency: '', 
      weekdays: weekdaysList[index],
    );
    ref.read(pharmacyProvider.notifier).updateMedication(index, newMed);
  }
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    if (pickedFile != null) {
      ...
      await ref.read(pharmacyProvider.notifier).scanPrescription(base64Image);
    }
  }
            ...
            children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: _buildHeaderAction(
                        ...
                        onTap: () => ref.read(pharmacyProvider.notifier).addNewMedication(),
                        isHighlight: false,
                      ),
                    ),
                  ],
              ...
              Expanded(
                child: state.medications.isEmpty
                    ? Center(
                        child: Text(
                          isArabic ? 'لا توجد أدوية حالياً. قم بإضافة دوائك الأول.' : 'No medications yet. Add your first one.',
                          style: TextStyle(
                            fontSize: 16,
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 16),
                      Text(
                        '${tr('pharmacy.medication')} ${index + 1}',
                        style: const TextStyle(
                      ...
                      onPressed: () async {
                        final confirm = await showDialog<bool>(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: Text(
                              isArabic 
                                ? 'هل أنت متأكد من رغبتتك في حذف هذا الدواء؟' 
                        ...
                        if (confirm == true) {
                          ref.read(pharmacyProvider.notifier).removeMedication(index);
                        }
                      },
                    ),
                  ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: ListTile(
                      ...
                      title: Text(tr('pharmacy.time'), style: const TextStyle(fontWeight: FontWeight.w600)),
                      subtitle: Text(
                        times[index] != null ? times[index]!.format(context) : tr('pharmacy.select_time'),
                        style: TextStyle(
                            color: times[index] != null ? AppColors.primary : Colors.grey,
                            fontWeight: FontWeight.bold),
                      ),
                      trailing: const Icon(Icons.access_time_filled, color: AppColors.primary),
                      onTap: () => _selectTime(context, index),
                    ),
                  ),
                  const SizedBox(height: 24),
                  Text(
                    isArabic ? 'جدول الأيام' : 'Schedule Days',
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              shape: BoxShape.circle,
              border: Border.all(
                color: isSelected ? AppColors.highlight : Colors.grey.shade300,
                width: 2,
              ),
            ),
            child: Center(
          ...
          children: [
              ...
              onPressed: () async {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                          isArabic ? 'تم حفظ الأدوية بنجاح' : 'Medications saved successfully',
                          style: const TextStyle(fontWeight: FontWeight.bold)),
                      backgroundColor: AppColors.primary,
            ...
            ElevatedButton.icon(
              onPressed: () async {
                ...
                try {
                  final optimizedMeds = await notifier.suggestSchedule();
                  if (optimizedMeds != null && mounted) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => AiScheduleScreen(medications: optimizedMeds),
                      ),
                    );
                  }
                } catch (e) {
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          isArabic ? 'حدث خطأ أثناء الاتصال بالذكاء الاصطناعي' : 'Error connecting to AI: $e',
                        ),
                        backgroundColor: Colors.redAccent,
                      ),
                    );
